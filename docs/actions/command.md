...
The `environment` attribute allows specific environment variables to be set for the child process without modifying the surrounding shell environment. The provided key/value pairs are merged on top of the variables inherited from the Terraform process, so existing variables such as `PATH` remain available.
...
- `environment` (Map of String) Environment variables to set for the command. These are merged with the environment variables inherited from the Terraform process, with these values taking precedence.
